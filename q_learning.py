...
# Code based off of https://github.com/studywolf/blog/tree/master/RL/Cat vs Mouse exploration
# 
class QLearn:
    ...
    def Rollout(self, Model, state, action, discount, policy, K, T):
        clf = svm.LinearSVC()
        clf.fit(self.trainData, self.trainLabels)
        q =  np.zeros(K)
        for k in range(K):
            value = clf.predict(state)
            value.frameNum = state.frameNum + 1;
            oldv = self.q.get((state, action), None)
            if oldv is None:
                self.q[(state, action)] = 1/value.frameNum;
            else:
                self.q[(state, action)] = oldv + self.alpha * (value - oldv)
